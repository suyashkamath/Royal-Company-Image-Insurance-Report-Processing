...
Hey , you are intelligent enough to identify which field belongs to what 

I am giving you the examples of How the data can be given 
so there can be the first two columns depicting the state and the city and the rest columns can be segment under that the payrates given 

Also keep in mind that there might be the same payrates in some rows , so please don't skip it by getting hallucinated 
Return ONLY JSON array, no markdown.

Sometimes , there can be multiple columns containing the values within them , means basically  a table is given , so please consider that too, what you do is first extract the whole text and then do it,

Sometimes the data is given in this format , wait , let me give you an example, so this depicts the multicolumnar - value



1. **Table Structure Understanding**
   - The first two columns (e.g., State, RTO Division) define the region.
   - The next columns represent numeric values (percentages) corresponding to payin or OD discount ranges.
   - Example columns: 
     - “12 to 20”
     - “12 to 20 @ 85% OD disc WO nil dep”
     - “20 to 40”
     - “20 to 40 @ 85% OD disc WO nil dep”
   - You must extract each such column as a **separate record** with the corresponding “range” and “od_disc_wo_nil_dep” values.

2. **Data Expansion**
   - For each row (State + RTO Division), create one record per numeric column.
   - Example:
     ```
     State = Gujarat
     RTO Division = Ahmedabad
     12 to 20 = 25%
     12 to 20@85% OD disc WO nil dep = 28%
     20 to 40 = 25%
     20 to 40@85% OD disc WO nil dep = 28%
     ```
   
    
3. **Data Normalization**
   - Convert all percentages to **numbers** (25% → 25).
   - Always include all visible rows (even duplicates).
   - If a cell is empty, use an empty string "".
   - Maintain exact capitalization of state and RTO division names.

4. **Formatting**
   - Return only a valid JSON array `[...]` with no markdown, no explanation, no extra text.
   - Validate the JSON before finalizing output.

---

Now extract and return the full table data as JSON following the above rules.
"""
